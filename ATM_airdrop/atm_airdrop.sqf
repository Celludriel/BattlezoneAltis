...
if (time < vHaloTimer) then {
	systemChat "Halo jump not available.";
	breakOut "main";
};
...
if ((getPos _target select 2) >= 8000) then{
	...
	{ if ((isFormationLeader player)&&(!isPlayer _x)&&(_x distance HaloFlag < 100)) then {
		_x setPosATL [ ((getPos player)select 0) + _ttt, (getPos player)select 1, 100];
		_para = createVehicle ["NonSteerable_Parachute_F", position _x, [], (((getDir player)-25)+(random 50)), 'NONE'];
...
while {(getPos _target select 2) > 2} do {
	...
	if (getPos _target select 2 < 130) then {
