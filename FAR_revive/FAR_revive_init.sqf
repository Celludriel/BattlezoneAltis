...
if (!isNil {_this select 0}) then { FAR_customRespawnScript = _this select 0; };
diag_log format ["FAR_customRespawnScript: %1", FAR_customRespawnScript];
...
FAR_Player_Init =
{
	...
	if(!isNil "FAR_customRespawnScript") then {
		[] call compile PreprocessFileLineNumbers FAR_customRespawnScript;
	};
